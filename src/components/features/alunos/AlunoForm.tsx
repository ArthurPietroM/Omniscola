...
export default function AlunoForm({ institutionId }: { institutionId: string }) {
  ...
  return (
    <div className="bg-white border border-black/[.06] rounded-xl p-4">
      <h2 className="text-sm font-medium text-[#003B6F] mb-1">Novo aluno</h2>
      <p className="text-xs text-gray-400 mb-3">A matrícula é gerada automaticamente pelo sistema.</p>
      ...
      <form ref={formRef} action={action} className="grid grid-cols-3 gap-3">
        ...
        <div className="flex items-end">
          <button
            type="submit"
            disabled={pending}
            className="w-full bg-[#E8601C] hover:bg-[#cf5418] disabled:opacity-60 text-white rounded-lg px-4 py-2 text-sm font-medium transition-colors"
          >
